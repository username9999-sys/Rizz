...
- **JWT Revocation** – Tokens include a `jti` (JWT ID) claim; access tokens can be revoked via Redis blacklist. Refresh tokens are rotated on each use and can also be revoked.
- **Email Verification** – Verification tokens are signed with `itsdangerous` and expire after 24 hours.
- **Password Reset** – Reset tokens are signed with `itsdangerous` and expire after 1 hour.
- **Audit Logging** – All privileged actions (register, login, logout, token refresh, email verification, password reset, post CRUD) emit structured JSON logs to the audit log table.
...
- Run a third‑party security audit and incorporate findings.
